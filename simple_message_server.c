...
static void handleChildSignals(int s);
...
int main(int argc,char *argv[]) {
	...
	while ((args = getopt(argc, argv, "p:h")) != -1) //getopt parses the commandline arguments
	{
		...
		{
			case 'p': 
				port = optarg; //return value of getopt, Assignment to port
				if(port == NULL)
				{
					printUsage(stderr, argv[0], EXIT_FAILURE);
				}
				errno = 0;
				portNumber = strtol(port, &endptr, 10);//converts the initial part of port into a int, according base 10
				if(errno != 0) {
					printUsage(stderr, argv[0], EXIT_FAILURE);
				}
				if(*endptr != 0) {
...
static int createSocket(char *port) {
	...
	for(p = addrInfo; p != NULL; p = p->ai_next) {
		...
		if (setsockopt(socketfd, SOL_SOCKET, SO_REUSEADDR, &true, sizeof(int)) == -1) {
			close(socketfd);
	...
	if (listen(socketfd, BACKLOG) == -1) { 
		...
		close(socketfd);//Fehlerhandling
		exit(EXIT_FAILURE);
	}

	/* set SignalAction for Child  - prevents Zombie-Processes*/
	signalAction.sa_handler = handleChildSignals;
...
static void execServer(int socketfd) { 
	...
	socklen_t cliSockSize;
	int newSocketfd = -1;

	while(1) { //runs the accept()and fork() SysCall in a loop
		cliSockSize = sizeof(cliSockAddr);

		newSocketfd = accept(socketfd, (struct sockaddr *) &cliSockAddr, &cliSockSize); //accept the connection from the client
		if (newSocketfd == -1) {
